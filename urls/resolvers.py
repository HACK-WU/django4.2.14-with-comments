...
class URLResolver:
    ...
    def resolve(self, path):
        """
        根据给定的路径解析URL模式匹配，并返回匹配结果。

        :param path: 需要解析的路径，可能是reverse_lazy对象
        :return: ResolverMatch对象，包含匹配结果和相关参数
        :raises Resolver404: 如果无法解析给定路径，则抛出此异常
        """
        # 将path转换为字符串格式，以处理reverse_lazy对象
        path = str(path)
        # 初始化一个空列表，用于存储尝试过的路径解析尝试
        tried = []
        # 尝试使用当前的URL模式匹配给定路径
        match = self.pattern.match(path)
        if match:
            # 如果匹配成功，解包匹配结果为新路径、参数和关键字参数
