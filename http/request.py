...
class QueryDict(MultiValueDict):
    """
    一个特化的 MultiValueDict，用于表示查询字符串。

    QueryDict 可以用来表示 GET 或 POST 数据。它继承自 MultiValueDict，因为此类数据中的键可以重复，
    例如在包含 <select multiple> 字段的表单数据中。

    默认情况下，QueryDict 是不可变的，但 copy() 方法将始终返回一个可变的副本。

    该类设置的键和值都会从给定的编码（默认为 DEFAULT_CHARSET）转换为 str。
    """

    # 这两个变量将在 __init__ 中重置，但在这里在类级别上指定，
    # 以便在反序列化时具有有效的值
    _mutable = True
    _encoding = None
